...
SERVICE_CATEGORIES = [
    ...
    {"id": "Fetichisme", "name": "Fetichisme", "icon": "sparkles"},
    {"id": "Vetements", "name": "Vetements", "icon": "scissors"},
    {"id": "Autres", "name": "Toy / Autres", "icon": "dumbbell"},
...
class EnterpriseRegistrationRequest(BaseModel):
    ...
    payment_method: Optional[str] = "cash"  # "cash" or "stripe"
...
async def register_enterprise_owner(data: EnterpriseRegistrationRequest):
    ...
    ent_name = enterprise.get('business_name') or enterprise.get('name')
    admin_email_html = f"""
    <h2>Activation entreprise existante</h2>
    <p><strong>Entreprise:</strong> {ent_name}</p>
    <p><strong>Catégorie:</strong> {enterprise.get('category')}</p>
    <p><strong>Demandeur:</strong> {data.first_name} {data.last_name}</p>
    <p><strong>Email:</strong> {data.email}</p>
    <p><strong>Téléphone:</strong> {data.phone}</p>
    <p><strong>Paiement:</strong> {data.payment_method or 'cash'}</p>
    <p><strong>Statut:</strong> EN ATTENTE</p>
    <p>Connectez-vous au dashboard admin pour valider cette inscription.</p>
    """
    asyncio.create_task(send_email("info@e-business-pay.com", f"Activation entreprise: {ent_name}", admin_email_html))
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 12px; overflow: hidden;">
        <div style="background: linear-gradient(135deg, #0047AB, #D4AF37); padding: 30px; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">Bienvenue sur Titelli !</h1>
        </div>
        <div style="padding: 30px;">
            <p style="font-size: 16px; color: #333;">Bonjour <strong>{data.first_name} {data.last_name}</strong>,</p>
            <p style="color: #555; line-height: 1.6;">Votre demande d'activation pour <strong>{ent_name}</strong> a bien été enregistrée.</p>
            <div style="background: #f8f9fa; border-radius: 8px; padding: 20px; margin: 20px 0;">
                <p style="color: #0047AB; font-weight: bold; margin: 0 0 10px 0;">Prochaines étapes :</p>
                <p style="color: #555; margin: 5px 0;">1. Vérification de vos documents</p>
                <p style="color: #555; margin: 5px 0;">2. Validation de votre identité</p>
                <p style="color: #555; margin: 5px 0;">3. Activation de votre compte (30-60 jours)</p>
                <p style="color: #555; margin: 5px 0;">4. Notification par email</p>
            </div>
            <p style="color: #555; line-height: 1.6;">Pour toute question, contactez-nous au <strong>+41 79 895 03 13</strong></p>
        </div>
        <div style="background: #f1f1f1; padding: 15px; text-align: center;">
            <p style="color: #999; font-size: 12px; margin: 0;">© 2026 Titelli - titelli.ch</p>
        </div>
    </div>
    """
    asyncio.create_task(send_email(data.email, "Confirmation d'inscription - Titelli", client_email_html))
...
# Model for new enterprise registration
class NewEnterpriseRegistrationRequest(BaseModel):
    # Enterprise info
    business_name: str
    category: str
    subcategory: Optional[str] = None
    ...
    first_name: str
    last_name: str
    commerce_register_id: Optional[str] = None
    manager_id: Optional[str] = None
    identity_document: Optional[str] = None
    payment_method: Optional[str] = "cash"  # "cash" or "stripe"

@api_router.post("/auth/register-new-enterprise")
async def register_new_enterprise(data: NewEnterpriseRegistrationRequest):
    """Register a completely new enterprise (not yet listed) with status 'en_attente'"""
    ...
    existing_enterprise = await db.enterprises.find_one({"business_name": {"$regex": f"^{data.business_name}$", "$options": "i"}})
    if existing_enterprise:
        raise HTTPException(status_code=400, detail="Une entreprise avec ce nom existe déjà")
    
    # Create new enterprise with status "en_attente"
    ...
    enterprise_dict = {
        ...
        "phone": data.phone,
        "email": data.email,
        "activation_status": "en_attente",  # Status: en_attente (waiting for validation)
        "display_status": "en_attente",
        "payment_method": data.payment_method or "cash",  # "cash" or "stripe"
        "payment_status": "pending",  # "pending" or "paid"
        "is_visible": False,  # Not visible until validated
        "is_certified": False,
        "is_labeled": False,
        "is_premium": False,
        "rating": 0,
    ...
    registration_request = {
        ...
        "identity_document": data.identity_document,
        "status": "pending",
        "is_new_enterprise": True,  # Flag to identify new enterprise registrations
        "created_at": datetime.now(timezone.utc).isoformat(),
        "user_info": {
            "email": data.email,
            "first_name": data.first_name,
    ...
    # Update enterprise with pending owner
    await db.enterprises.update_one(
        {"id": enterprise_id},
        {"$set": {
            "pending_owner_id": user_id,
    ...
    # Send webhook to SalonPro/Titelli Management
    sync_data = {
        "enterprise_id": enterprise_id,
        "user_id": user_id,
        "business_name": data.business_name,
    ...
    asyncio.create_task(send_webhook_to_salonpro("new_enterprise_created", sync_data))
    
    # Send email notification to admin
    admin_email_html = f"""
    <h2>Nouvelle inscription entreprise</h2>
    <p><strong>Entreprise:</strong> {data.business_name}</p>
    ...
    <p>Connectez-vous au dashboard admin pour valider cette inscription.</p>
    """
    asyncio.create_task(send_email("info@e-business-pay.com", f"Nouvelle inscription: {data.business_name}", admin_email_html))
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 12px; overflow: hidden;">
        ...
        <div style="padding: 30px;">
            <p style="font-size: 16px; color: #333;">Bonjour <strong>{data.first_name} {data.last_name}</strong>,</p>
            <p style="color: #555; line-height: 1.6;">Votre demande d'inscription pour <strong>{data.business_name}</strong> a bien été enregistrée.</p>
            <div style="background: #f8f9fa; border-radius: 8px; padding: 20px; margin: 20px 0;">
                <p style="color: #0047AB; font-weight: bold; margin: 0 0 10px 0;">Prochaines étapes :</p>
                <p style="color: #555; margin: 5px 0;">1. Vérification de vos documents</p>
                <p style="color: #555; margin: 5px 0;">2. Validation de votre identité</p>
    ...
    logger.info(f"New enterprise registration: {data.business_name} - Status: en_attente")
    
    return {
        "success": True,
        "message": "Votre demande d'inscription a été enregistrée. Votre entreprise sera visible après validation.",
        "request_id": registration_request["id"],
        "enterprise_id": enterprise_id,
        "status": "en_attente",
        "free_ai_credit": True
...
# Stripe checkout for enterprise registration payment
@api_router.post("/registration/checkout-stripe")
async def registration_stripe_checkout(request: Request, enterprise_id: str = "", request_id: str = ""):
    """Create a Stripe checkout session for enterprise registration fee (250 CHF)"""
    ...
    if 'X-Forwarded-Host' in request.headers:
        host_url = f"https://{request.headers['X-Forwarded-Host']}"
    
    webhook_url = f"{host_url}/api/webhook/stripe"
    stripe_checkout = StripeCheckout(api_key=STRIPE_API_KEY, webhook_url=webhook_url)
    
    ent_name = enterprise.get("business_name") or enterprise.get("name", "Entreprise")
    
    checkout_request = CheckoutSessionRequest(
        line_items=[{
            "price_data": {
                "currency": "chf",
                "product_data": {"name": f"Inscription Titelli - {ent_name}"},
                "unit_amount": 25000  # 250 CHF in centimes
            },
            "quantity": 1
        }],
        success_url=f"{host_url}/inscription-entreprise?payment=success&enterprise_id={enterprise_id}",
        cancel_url=f"{host_url}/inscription-entreprise?payment=cancel",
        metadata={
    ...
    try:
        ...
        await db.enterprises.update_one(
            {"id": enterprise_id},
            {"$set": {"payment_method": "stripe", "stripe_session_id": session.session_id}}
        )
        
        return {"url": session.url, "session_id": session.session_id}
    except Exception as e:
        logger.error(f"Stripe registration checkout error: {e}")
        raise HTTPException(status_code=500, detail="Erreur lors de la création du paiement")

# Admin: Validate cash payment
@api_router.post("/admin/registration-requests/{request_id}/validate-payment")
async def validate_cash_payment(
    request_id: str,
    current_user: dict = Depends(get_current_user)
):
    """Mark cash payment as received for a registration request"""
    if current_user.get("user_type") != "admin":
    ...
    await db.enterprises.update_one(
        ...
        {"$set": {
            "payment_status": "paid",
            "payment_validated_by": current_user.get("id"),
            "payment_validated_at": datetime.now(timezone.utc).isoformat()
        }}
    )
    ...
    await db.registration_requests.update_one(
        {"id": request_id},
        {"$set": {"payment_status": "paid", "payment_validated_at": datetime.now(timezone.utc).isoformat()}}
    )
...
async def get_registration_requests(
    ...
    # Enrich with enterprise data
    for req in requests:
        enterprise = await db.enterprises.find_one({"id": req.get("enterprise_id")}, {"_id": 0, "name": 1, "category": 1, "address": 1, "image": 1, "payment_method": 1, "payment_status": 1, "business_name": 1, "city": 1})
...
# ============ WEBSITE REQUESTS ============
@api_router.post("/website-requests")
async def create_website_request(request: Request):
    """Save a website creation request from enterprise registration"""
    ...
    await db.siteweb.insert_one(website_request)
    
    # Send notification email
    admin_html = f"""
    <h2>Nouvelle demande de site web</h2>
    <p><strong>Entreprise:</strong> {data.get('business_name')}</p>
    <p><strong>Email:</strong> {data.get('email')}</p>
    <p><strong>Telephone:</strong> {data.get('phone')}</p>
    <p><strong>Couleur preferee:</strong> {data.get('preferred_color')}</p>
    <p><strong>Images:</strong> {data.get('images_description')}</p>
    <p><strong>Description site:</strong> {data.get('site_description')}</p>
    """
    asyncio.create_task(send_email("info@e-business-pay.com", f"Demande site web: {data.get('business_name')}", admin_html))
    
    return {"success": True, "message": "Demande enregistree", "id": website_request["id"]}

@api_router.get("/admin/website-requests")
async def get_website_requests(current_user: dict = Depends(get_current_user)):
    """Get all website requests (admin only)"""
    if current_user.get("user_type") != "admin":
        raise HTTPException(status_code=403, detail="Acces reserve aux administrateurs")
    
    requests_list = await db.siteweb.find({}, {"_id": 0}).sort("created_at", -1).to_list(100)
