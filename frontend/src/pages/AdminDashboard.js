...
const ADMIN_EMAILS = ['admin@pinkgirl.com', 'spa.luxury@pinkgirl.com', 'admin@pinkgirl.ch','admin@titelli.com'];
...
const AdminDashboard = () => {
  ...
  const handleValidatePayment = async (requestId) => {
    try {
      await axios.post(`${API_URL}/admin/registration-requests/${requestId}/validate-payment`, null, {
        headers: { Authorization: `Bearer ${localStorage.getItem('titelli_token')}` }
      });
      toast.success('Paiement cash validé !');
      fetchRegistrationRequests();
    } catch (error) {
      toast.error(error.response?.data?.detail || 'Erreur lors de la validation du paiement');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#FFF5F9] pt-20" data-testid="admin-dashboard">
      <div className="flex">
        {/* Sidebar */}
        <aside className="w-64 min-h-screen bg-white border-r border-white/5 fixed left-0 top-20 bottom-0 overflow-y-auto hidden lg:block">
          <div className="p-6">
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                    activeTab === item.id
                      ...
                      : 'text-gray-400 hover:bg-pink-50/50 hover:text-white'
                  }`}
                >
        ...
        <main className="flex-1 lg:ml-64 p-6 lg:p-8">
          {activeTab === 'overview' && stats && (
            <div className="space-y-8">
              <h1 className="text-2xl md:text-3xl font-bold text-white" style={{ fontFamily: 'Dancing Script, cursive' }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="card-service rounded-xl p-6">
                  <div className="p-3 rounded-xl bg-sky-400/20 w-fit mb-4">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="card-service rounded-xl p-6">
                  <h2 className="text-lg font-bold text-gray-900 mb-4">Utilisateurs récents</h2>
                  <div className="space-y-3">
                    {stats.recent_users?.slice(0, 5).map((u) => (
                      <div key={u.id} className="flex items-center justify-between p-3 bg-pink-50/50 rounded-lg">
                        <div>
                          <p className="text-gray-800 font-medium">{u.first_name} {u.last_name}</p>
                          <p className="text-sm text-gray-400">{u.email}</p>
                        </div>
                        <span className={`px-2 py-1 rounded-full text-xs ${
                          u.user_type === 'entreprise' ? 'bg-sky-400/20 text-sky-400' : 'bg-pink-500/20 text-pink-500'
                        }`}>
                          {u.user_type}
                        </span>
                      </div>
                ...
                <div className="card-service rounded-xl p-6">
                  ...
                  <div className="space-y-3">
                    {stats.recent_orders?.length > 0 ? stats.recent_orders.slice(0, 5).map((o) => (
                      <div key={o.id} className="flex items-center justify-between p-3 bg-pink-50/50 rounded-lg">
                        <div>
                          <p className="text-gray-800 font-medium">#{o.id.slice(0, 8)}</p>
                          <p className="text-sm text-gray-400">{o.items?.length || 0} articles</p>
                        </div>
                        <p className="text-gray-800 font-medium">{o.total?.toFixed(2)} CHF</p>
                      </div>
                    )) : (
            ...
            <div>
              <div className="flex items-center justify-between mb-8">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-pink-50 text-white rounded-lg hover:bg-white/20 transition-colors"
                >
              ...
              {registrationLoading ? (
                ...
                <div className="space-y-4">
                  {registrationRequests.map((request) => {
                    const isNew = request.is_new_enterprise;
                    const entName = request.enterprise_name || request.enterprise_info?.business_name || request.enterprise?.name || 'Entreprise inconnue';
                    const entCategory = request.enterprise_info?.category || request.enterprise?.category || '-';
                    const entAddress = request.enterprise_info?.address || request.enterprise?.address || '-';
                    const entCity = request.enterprise_info?.city || '';
                    return (
                    <div key={request.id} className="bg-pink-50/50 border border-pink-100 rounded-xl p-6" data-testid={`reg-request-${request.id}`}>
                      {/* Type badge */}
                      <div className="flex items-center gap-2 mb-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${isNew ? 'bg-sky-400/20 text-sky-400' : 'bg-pink-500/20 text-pink-500'}`}>
                          {isNew ? 'Nouvelle entreprise' : 'Activation existante'}
                        </span>
                      ...
                      <div className="flex flex-col lg:flex-row lg:items-start gap-6">
                        ...
                        <div className="flex items-start gap-4 flex-1">
                          <img
                            src={request.enterprise?.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(entName)}&background=0047AB&color=fff`}
                            alt={entName}
                            className="w-16 h-16 rounded-lg object-cover"
                          />
                          <div>
                            <h3 className="text-gray-900 font-semibold text-lg">{entName}</h3>
                            <p className="text-sky-400 text-sm">{entCategory}</p>
                            <p className="text-gray-400 text-sm flex items-center gap-1 mt-1">
                              <MapPin className="w-4 h-4" />
                              {entAddress}{entCity ? `, ${entCity}` : ''}
                            </p>
                            {isNew && request.enterprise_info?.website && (
                              <a href={request.enterprise_info.website} target="_blank" rel="noopener noreferrer" className="text-blue-400 text-xs hover:underline mt-1 block">
                                {request.enterprise_info.website}
                              </a>
                            )}
                            {isNew && request.enterprise_info?.description && (
                              <p className="text-gray-500 text-xs mt-2 line-clamp-2">{request.enterprise_info.description}</p>
                            )}
                          </div>
                        </div>

                        {/* User Info */}
                        <div className="flex-1 bg-pink-50/50 rounded-lg p-4">
                          <h4 className="text-white font-medium mb-3">Demandeur</h4>
                        ...
                        <div className="flex-1 bg-pink-50/50 rounded-lg p-4">
                          ...
                          <div className="space-y-2 text-sm">
                            <p className="text-gray-600">
                              ...
                              <span className="text-sky-400 font-mono">{request.commerce_register_id || '-'}</span>
                            </p>
                            <p className="text-gray-600">
                            ...
                            <p className="text-gray-600">
                              ...
                              {request.identity_document ? (
                                <span className="text-green-400">Fournie</span>
                              ) : (
                      ...
                      {/* Payment status */}
                      <div className="mt-4 flex items-center gap-3 px-4 py-3 bg-pink-50/50 rounded-lg">
                        <span className="text-gray-500 text-sm">Paiement:</span>
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          (request.enterprise?.payment_status || request.payment_status) === 'paid' 
                            ? 'bg-green-500/20 text-green-400' 
                            : 'bg-yellow-500/20 text-yellow-400'
                        }`}>
                          {(request.enterprise?.payment_status || request.payment_status) === 'paid' ? 'Payé' : 'En attente'}
                        </span>
                        <span className="text-gray-400 text-xs">
                          ({request.enterprise?.payment_method || 'cash'})
                        </span>
                        {(request.enterprise?.payment_status || request.payment_status) !== 'paid' && 
                         (request.enterprise?.payment_method || 'cash') === 'cash' && (
                          <button
                            onClick={() => handleValidatePayment(request.id)}
                            className="ml-auto flex items-center gap-1 px-3 py-1 bg-sky-400/20 text-sky-400 rounded-lg hover:bg-sky-400/30 text-xs font-medium transition-colors"
                            data-testid={`validate-payment-btn-${request.id}`}
                          >
                      ...
                      {/* Actions */}
                      <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-pink-100">
                        <button
          ...
          {activeTab === 'pub-media' && (
            <div data-testid="admin-pub-media">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                      pubMediaFilter === status 
                        ...
                        : 'bg-pink-50 text-gray-400 hover:bg-white/20'
                    }`}
                  >
              ...
              {pubMediaLoading ? (
                ...
                <div className="bg-pink-50/50 border border-pink-100 rounded-xl overflow-hidden">
                      ...
                      <tbody className="divide-y divide-white/5">
                        {pubMediaOrders.map(order => (
                          <tr key={order.id} className="hover:bg-pink-50/50 transition-colors">
                            ...
                            <td className="p-4">
                              <span className="text-gray-700 text-sm">{order.template_name || 'Sur Mesure'}</span>
                              {order.template_id === 'sur_mesure' && (
                                <span className="ml-2 px-2 py-0.5 bg-purple-500/20 text-purple-400 text-xs rounded">Custom</span>
                              )}
                            </td>
                            <td className="p-4">
                              <span className="text-gray-600 text-sm">{order.enterprise_id?.slice(0, 8) || 'N/A'}</span>
                            </td>
                            <td className="p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {algorithms.map((algo) => {
                    ...
                    return (
                      <div 
                        key={algo.id} 
                        className={`bg-pink-50/50 border rounded-xl p-5 transition-all ${
                          algo.enabled ? 'border-green-500/30' : 'border-pink-100 opacity-60'
                        }`}
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-lg ${algo.enabled ? 'bg-green-500/20 text-green-400' : 'bg-pink-50 text-gray-400'}`}>
                              {categoryIcons[algo.category] || <Cpu className="w-5 h-5" />}
                            </div>
                            <div>
                              <h3 className="text-white font-semibold">{algo.name}</h3>
                              <span className="text-xs text-sky-400">{algo.category}</span>
                            </div>
                          </div>
                          <button
              ...
              {subscriptionLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {subscriptionPlans.map((plan) => (
                    <div 
                      key={plan.id} 
                      className={`bg-pink-50/50 border rounded-xl p-6 transition-all ${
                        plan.id === 'premium' ? 'border-sky-300/50 ring-1 ring-[#D4AF37]/20' : 
                        plan.id === 'enterprise' ? 'border-pink-400/50' : 'border-pink-100'
                      }`}
                    >
                      {editingPlan === plan.id ? (
                        // Edit mode
                        <div className="space-y-4">
                          <input
                            ...
                            className="w-full px-3 py-2 bg-pink-50 border border-pink-200 rounded-lg text-white"
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-pink-50 text-white rounded-lg hover:bg-white/20"
                            >
                              Annuler
                            </button>
                          </div>
                        ...
                        <>
                          <div className="flex items-center justify-between mb-4">
                            ...
                            <button
                              ...
                              className="p-2 text-gray-400 hover:text-gray-700 hover:bg-pink-50 rounded-lg transition-colors"
                            >
              ...
              <div className="card-service rounded-xl p-4 mb-6">
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <input
                    ...
                    className="px-3 py-2 bg-pink-50 border border-pink-200 rounded-lg text-gray-700 text-sm"
                  />
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-pink-500 text-white rounded-lg font-medium hover:bg-[#003d91] transition-colors"
                  >
              ...
              {loadingAccounting ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    ...
                    <div className="card-service rounded-xl p-6">
                      <div className="flex items-center justify-between mb-4">
                        <div className="p-3 rounded-xl bg-sky-400/20">
                      ...
                      <p className="text-sm text-gray-400 mt-1">Commissions PinkGirl</p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                    ...
                    <div className="card-service rounded-xl p-6">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center p-3 bg-pink-50/50 rounded-lg">
                          <span className="text-gray-600">Ventes (commandes)</span>
                          <span className="text-gray-800 font-medium">{accountingSummary.revenue?.total_orders_revenue?.toFixed(2)} CHF</span>
                        </div>
                        <div className="flex justify-between items-center p-3 bg-pink-50/50 rounded-lg">
                          <span className="text-gray-600">Abonnements</span>
                          <span className="text-gray-800 font-medium">{accountingSummary.revenue?.subscription_revenue?.toFixed(2)} CHF</span>
                        </div>
                    ...
                    <div className="card-service rounded-xl p-6">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center p-3 bg-pink-50/50 rounded-lg">
                          <span className="text-gray-600">Commission 5% (commandes)</span>
                          <span className="text-gray-800 font-medium">{accountingSummary.commissions?.order_commissions_5pct?.toFixed(2)} CHF</span>
                        </div>
                        <div className="flex justify-between items-center p-3 bg-pink-50/50 rounded-lg">
                          <span className="text-gray-600">Frais de gestion</span>
                          <span className="text-gray-800 font-medium">{accountingSummary.commissions?.management_fees?.toFixed(2)} CHF</span>
                        </div>
                        <div className="flex justify-between items-center p-3 bg-pink-50/50 rounded-lg">
                          <span className="text-gray-600">Commission 12% (investissements)</span>
                          <span className="text-gray-800 font-medium">{accountingSummary.commissions?.investment_commissions_12pct?.toFixed(2)} CHF</span>
                        </div>
                        <div className="flex justify-between items-center p-3 bg-sky-400/10 rounded-lg border border-sky-300/30">
                          <span className="text-sky-400 font-medium">Total</span>
                          <span className="text-sky-400 font-bold">{accountingSummary.commissions?.total_commissions?.toFixed(2)} CHF</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="card-service rounded-xl p-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between items-center p-3 bg-pink-50/50 rounded-lg">
                          ...
                          <span className="text-gray-800 font-medium">{accountingSummary.subscriptions?.active_count}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className={`px-3 py-1 rounded-lg text-sm ${!transactionFilter ? 'bg-pink-500 text-white' : 'bg-pink-50 text-gray-600'}`}
                          >
                            Tous
                          </button>
                          <button
                            onClick={() => { setTransactionFilter('order'); fetchAccountingData(); }}
                            className={`px-3 py-1 rounded-lg text-sm ${transactionFilter === 'order' ? 'bg-pink-500 text-white' : 'bg-pink-50 text-gray-600'}`}
                          >
                            Commandes
                          </button>
                          <button
                            onClick={() => { setTransactionFilter('subscription'); fetchAccountingData(); }}
                            className={`px-3 py-1 rounded-lg text-sm ${transactionFilter === 'subscription' ? 'bg-pink-500 text-white' : 'bg-pink-50 text-gray-600'}`}
                          >
                            Abonnements
                          </button>
                          <button
                            onClick={() => { setTransactionFilter('cashback'); fetchAccountingData(); }}
                            className={`px-3 py-1 rounded-lg text-sm ${transactionFilter === 'cashback' ? 'bg-pink-500 text-white' : 'bg-pink-50 text-gray-600'}`}
                          >
                            Cashback
                          </button>
                        </div>
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-white/5">
                        {accountingTransactions.slice(0, 50).map((tx, idx) => (
                          <tr key={tx.id || idx} className="hover:bg-pink-50/50">
                            ...
                            <td className="px-6 py-4 text-sm text-sky-400 text-right">
            ...
            <div>
              <div className="flex items-center justify-between mb-8">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-sky-400 text-black rounded-lg font-medium hover:bg-[#C4A030] transition-colors"
                >
              ...
              <div className="card-service rounded-xl overflow-hidden">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {withdrawals.map((w) => (
                      <tr key={w.id} className="hover:bg-pink-50/50">
                        <td className="px-6 py-4">
                          <p className="text-gray-700 text-sm">{new Date(w.created_at).toLocaleDateString('fr-FR')}</p>
                          <p className="text-gray-500 text-xs">{new Date(w.created_at).toLocaleTimeString('fr-FR')}</p>
                        </td>
                        <td className="px-6 py-4">
                          <p className="text-gray-800 font-medium">{w.account_holder}</p>
                        ...
                        <td className="px-6 py-4">
                          <p className="text-sky-400 font-bold">{w.amount.toFixed(2)} CHF</p>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className="p-2 bg-pink-50 text-white rounded-lg hover:bg-white/20 transition-colors"
                              title="Voir détails"
                            >
                              <Eye className="w-4 h-4" />
                            </button>
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-xl font-bold text-sky-400">{selectedWithdrawal.amount.toFixed(2)} CHF</p>
                        </div>
                        <div>
                      ...
                      <div>
                        ...
                        <p className="text-gray-800 font-medium">{selectedWithdrawal.account_holder}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm text-gray-400">IBAN (complet)</p>
                        <p className="text-white font-mono bg-pink-50 p-2 rounded">{selectedWithdrawal.iban}</p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-gray-900">{new Date(selectedWithdrawal.created_at).toLocaleString('fr-FR')}</p>
                      </div>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-3 py-1 bg-sky-400/20 text-sky-400 rounded-lg text-xs hover:bg-sky-400/30"
                            >
                              Certifier
                            </button>
                            <button
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {stats?.recent_enterprises?.map((e) => (
                      <tr key={e.id} className="hover:bg-pink-50/50">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            {e.logo ? (
                            ...
                            <div>
                              <p className="text-gray-800 font-medium">{e.business_name}</p>
                              <p className="text-gray-400 text-sm">{e.city || 'Non spécifié'}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-gray-600">{e.category || 'N/A'}</td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-full text-xs ${
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {stats?.recent_orders?.map((o) => (
                      <tr key={o.id} className="hover:bg-pink-50/50">
                        <td className="px-6 py-4">
                          <p className="text-gray-800 font-medium">#{o.id?.slice(0, 8)}</p>
                        </td>
                        <td className="px-6 py-4 text-gray-600">{o.user_id?.slice(0, 8) || 'N/A'}</td>
                        <td className="px-6 py-4 text-sky-400 font-medium">{o.total?.toFixed(2)} CHF</td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-full text-xs ${
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="card-service rounded-xl p-6">
                  ...
                  <p className="text-3xl font-bold text-sky-400">{accountingSummary?.commissions?.total_commissions?.toFixed(2) || '0.00'} CHF</p>
                </div>
                <div className="card-service rounded-xl p-6">
              ...
              <div className="card-service rounded-xl p-6">
                <p className="text-gray-400 text-center">
                  Pour voir les détails des transactions, consultez la section <button onClick={() => setActiveTab('accounting')} className="text-pink-500 hover:underline">Comptabilité</button>
                </p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'settings' && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="card-service rounded-xl p-6">
                  <h3 className="text-lg font-bold text-gray-900 mb-4">Taux de Commission</h3>
                  <div className="space-y-3">
                    <div className="flex justify-between p-3 bg-pink-50/50 rounded-lg">
                      <span className="text-gray-600">Commission sur commandes</span>
                      <span className="text-sky-400 font-bold">5%</span>
                    </div>
                    <div className="flex justify-between p-3 bg-pink-50/50 rounded-lg">
                      <span className="text-gray-600">Commission sur investissements</span>
                      <span className="text-sky-400 font-bold">12%</span>
                    </div>
                  </div>
                </div>
                <div className="card-service rounded-xl p-6">
                  <h3 className="text-lg font-bold text-gray-900 mb-4">Cashback Client</h3>
                  <div className="space-y-3">
                    <div className="flex justify-between p-3 bg-pink-50/50 rounded-lg">
                    ...
                    <div className="flex justify-between p-3 bg-pink-50/50 rounded-lg">
                      <span className="text-gray-600">Plan VIP</span>
                      <span className="text-green-500 font-bold">15%</span>
                    </div>
                  </div>
                </div>
                <div className="card-service rounded-xl p-6">
                  <h3 className="text-lg font-bold text-gray-900 mb-4">Retraits</h3>
                  <div className="space-y-3">
                    <div className="flex justify-between p-3 bg-pink-50/50 rounded-lg">
                      <span className="text-gray-600">Montant minimum</span>
                      <span className="text-gray-900 font-bold">50 CHF</span>
                    </div>
                    <div className="flex justify-between p-3 bg-pink-50/50 rounded-lg">
                      <span className="text-gray-600">Délai de traitement</span>
                      <span className="text-gray-900 font-bold">1-5 jours</span>
                    </div>
                  </div>
                </div>
                <div className="card-service rounded-xl p-6">
                  <h3 className="text-lg font-bold text-gray-900 mb-4">Admins Autorisés</h3>
                  <div className="space-y-3">
                    <div className="p-3 bg-pink-50/50 rounded-lg">
                      <span className="text-gray-600">admin@pinkgirl.com</span>
                    </div>
                    <div className="p-3 bg-pink-50/50 rounded-lg">
                      <span className="text-gray-600">spa.luxury@pinkgirl.com</span>
                    </div>
                  </div>
                </div>
              </div>
