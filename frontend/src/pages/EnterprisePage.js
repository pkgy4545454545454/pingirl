...
const EnterprisePage = () => {
  ...
  if (!enterprise) {
    return (
      <div className="min-h-screen bg-[#FFF5F9] pt-24 flex flex-col items-center justify-center">
        <h2 className="text-2xl font-bold text-gray-900 mb-4">Entreprise non trouvée</h2>
  ...
  return (
    <div className="min-h-screen bg-[#FFF5F9]" data-testid="enterprise-page">
      ...
      <div className="relative h-[50vh] md:h-[60vh] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-8 md:left-16 transform translate-y-1/2 z-30" style={{marginBottom:'90px'}}>
          <div className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-white shadow-lg overflow-hidden bg-pink-50 shadow-2xl hidden">
            {enterprise.logo ? (
              <img src={getImageUrl(enterprise.logo)} alt="" className="w-full h-full object-cover hidden" />
            ) : (
              <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-pink-400 to-[#D4AF37] hidden">
          ...
          <div className="absolute -right-2 -top-2 flex flex-col gap-1">
            ...
            {enterprise.is_labeled && (
              <span className="w-8 h-8 rounded-full bg-sky-400 flex items-center justify-center shadow-lg">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-24 pb-12">
        {/* Header Info */}
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8" style={{backgroundColor:'pink', padding:'10px'}}>
          <div className="md:pl-48">
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900" style={{ fontFamily: 'Dancing Script, cursive' }}>
              {enterprise.business_name}
            </h1>
            {enterprise.slogan && (
              <p className="text-sky-400 text-lg mt-1">{enterprise.slogan}</p>
            )}
            <div className="flex flex-wrap items-center gap-4 mt-3 text-gray-400">
              ...
              {enterprise.rating > 0 && (
                <div className="flex items-center gap-1">
                  <Star className="w-4 h-4 fill-[#D4AF37] text-sky-400" />
                  <span className="text-gray-800 font-medium">{enterprise.rating?.toFixed(1) || '0.0'}</span>
          ...
          <div className="flex items-center gap-3 md:mt-4">
            <button 
              ...
              className={`p-3 rounded-full transition-colors flex items-center gap-2 ${
                isMyProvider 
                  ...
                  : 'bg-pink-50/50 text-gray-500 hover:text-pink-500 hover:bg-pink-50'
              }`}
            ...
            <button className="p-3 bg-pink-50/50 rounded-full text-gray-400 hover:text-red-400 hover:bg-pink-50 transition-colors">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  activeTab === tab.id
                    ...
                    : 'bg-pink-50/50 text-gray-500 hover:text-pink-500'
                }`}
              >
                {tab.label}
              </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="card-service rounded-xl p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-4" style={{ fontFamily: 'Dancing Script, cursive' }}>
                À propos
              </h2>
            ...
            {(mediaGallery.photos.length > 0 || mediaGallery.videos.length > 0) && (
              <div className="card-service rounded-xl p-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-6" style={{ fontFamily: 'Dancing Script, cursive' }}>
                  ...
                  <div className="mb-8">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {mediaGallery.photos.map((photo, index) => (
                        <div 
                          key={photo.id || index} 
                          className="relative aspect-square rounded-xl overflow-hidden cursor-pointer hover:opacity-90 transition-opacity border border-pink-100 hover:border-pink-400/50 group"
                              ...
                              >
                                ...
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover/tag:opacity-100 transition-opacity pointer-events-none z-20">
                                  <div className="bg-black/90 backdrop-blur-sm rounded-lg px-3 py-2 whitespace-nowrap border border-pink-100 shadow-xl">
                                    ...
                                    <p className="text-xs text-pink-500 mt-1">Cliquer pour voir →</p>
                                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {mediaGallery.videos.map((video, index) => (
                        <div 
                          key={video.id || index} 
                          className="aspect-video rounded-xl overflow-hidden cursor-pointer hover:opacity-90 transition-opacity border border-pink-100 hover:border-sky-300/50 relative group"
              ...
              <div className="card-service rounded-xl p-6">
                ...
                <div className={displayVariant === 'side' ? 'space-y-4' : 'grid grid-cols-1 md:grid-cols-2 gap-4'}>
                  {services.filter(s => s.type === 'service').map((service) => (
                    displayVariant === 'side' ? (
                      // Side by Side Layout
                      <Link to={`/service/${service.id}`} key={service.id} className="flex gap-4 p-4 bg-pink-50/50 rounded-xl hover:bg-pink-50 transition-colors border border-transparent hover:border-pink-400/30">
                        <div className="w-32 h-24 rounded-lg bg-pink-50/50 overflow-hidden flex-shrink-0">
                          {service.images?.[0] ? (
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                          ...
                          <div className="flex items-center justify-between mt-2">
                            <p className="text-lg font-bold text-sky-400">{service.price?.toFixed(2)} CHF</p>
                            <button
                              onClick={(e) => { e.preventDefault(); handleAddToCart(service); }}
                              className="p-2 bg-pink-500 rounded-full text-white hover:bg-[#2E74D6] transition-colors"
                            >
                      ...
                      // Text Below Layout
                      <Link to={`/service/${service.id}`} key={service.id} className="bg-pink-50/50 rounded-xl overflow-hidden hover:bg-pink-50 transition-colors border border-transparent hover:border-pink-400/30">
                        <div className="aspect-video bg-pink-50/50 overflow-hidden">
                          {service.images?.[0] ? (
                            <img src={service.images[0]} alt={service.name} className="w-full h-full object-cover" />
                          ) : (
              ...
              <div className="card-service rounded-xl p-6">
                ...
                <div className={displayVariant === 'side' ? 'space-y-4' : 'grid grid-cols-1 md:grid-cols-2 gap-4'}>
                  {services.filter(s => s.type === 'product').map((product) => (
                    displayVariant === 'side' ? (
                      // Side by Side Layout
                      <Link to={`/product/${product.id}`} key={product.id} className="flex gap-4 p-4 bg-pink-50/50 rounded-xl hover:bg-pink-50 transition-colors border border-transparent hover:border-sky-300/30">
                        <div className="w-32 h-24 rounded-lg bg-pink-50/50 overflow-hidden flex-shrink-0">
                          {product.images?.[0] ? (
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                          ...
                          <div className="flex items-center justify-between mt-2">
                            <p className="text-lg font-bold text-sky-400">{product.price?.toFixed(2)} CHF</p>
                            <button
                              onClick={(e) => { e.preventDefault(); handleAddToCart(product); }}
                              className="p-2 bg-sky-400 rounded-full text-black hover:bg-[#F3CF55] transition-colors"
                            >
                              <ShoppingCart className="w-4 h-4" />
                            </button>
                          </div>
                      ...
                      // Text Below Layout
                      <Link to={`/product/${product.id}`} key={product.id} className="bg-pink-50/50 rounded-xl overflow-hidden hover:bg-pink-50 transition-colors border border-transparent hover:border-sky-300/30">
                        <div className="aspect-video bg-pink-50/50 overflow-hidden">
                          {product.images?.[0] ? (
                            <img src={product.images[0]} alt={product.name} className="w-full h-full object-cover" />
                          ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center gap-4 p-4 bg-pink-50/50 rounded-xl">
                    ...
                    <div>
                      ...
                      <p className="text-black">{enterprise.address}, {enterprise.city}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 p-4 bg-pink-50/50 rounded-xl">
                    ...
                    <div>
                      ...
                      <p className="text-black">{enterprise.phone}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 p-4 bg-pink-50/50 rounded-xl">
                    ...
                    <div>
                      ...
                      <p className="text-black">{enterprise.email}</p>
                    </div>
                  </div>
                  {enterprise.website && (
                    <div className="flex items-center gap-4 p-4 bg-pink-50/50 rounded-xl">
                      ...
                      <div>
                        ...
                        <a href={enterprise.website} target="_blank" rel="noopener noreferrer" className="text-pink-500 hover:underline">
                          {enterprise.website}
                        </a>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {trainings.map((training) => (
                      <Link 
                        ...
                        className="bg-pink-50/50 rounded-xl overflow-hidden hover:bg-pink-50 transition-colors border border-transparent hover:border-[#10B981]/30 group"
                      >
                  ...
                  <div className="space-y-4">
                    {jobs.map((job) => (
                      <Link 
                        key={job.id} 
                        to={`/emploi/${job.id}`}
                        className="block p-4 bg-pink-50/50 rounded-xl hover:bg-pink-50 transition-colors border border-transparent hover:border-[#8B5CF6]/30"
                      >
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                {enterprise?.phone && (
                  <a 
                    ...
                    className="w-full py-3 px-4 bg-yellow-70/30 text-black rounded-lg font-medium hover:bg-pink-50 transition-colors flex items-center justify-center gap-2"
                  >
            ...
            {/* All Reviews */}
            <div className="card-service rounded-xl p-6 hidden">
                ...
                {reviews.length > 0 ? reviews.map((review) => (
                  <div key={review.id} className="pb-4 border-b border-white/5 last:border-0">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-1">
                        {[...Array(5)].map((_, i) => (
                          <Star key={i} className={`w-3 h-3 ${i < review.rating ? 'fill-[#D4AF37] text-sky-400' : 'text-gray-600'}`} />
                        ))}
                      </div>
                    </div>
              ...
              {/* Add Review Form */}
              {isAuthenticated && (
                <form onSubmit={handleSubmitReview} className="pt-4 mt-4 border-t border-pink-100">
                  ...
                  <div className="flex items-center gap-1 mb-3">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button key={star} type="button" onClick={() => setNewReview({ ...newReview, rating: star })}>
                        <Star className={`w-6 h-6 cursor-pointer transition-colors ${star <= newReview.rating ? 'fill-[#D4AF37] text-sky-400' : 'text-gray-600 hover:text-sky-400'}`} />
                      </button>
                    ))}
                  </div>
                  <textarea
      ...
      {selectedMedia && (
        ...
        >
          <button 
            className="absolute top-6 right-6 p-3 bg-pink-50 rounded-full text-white hover:bg-white/20 transition-colors"
            onClick={() => setSelectedMedia(null)}
          >
            <X className="w-6 h-6" />
          </button>
