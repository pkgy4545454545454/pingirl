...
const OrdersPage = () => {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-[#FFF5F9] pt-24 px-4" data-testid="orders-page">
  ...
  return (
    <div className="min-h-screen bg-[#FFF5F9] pt-24 px-4" data-testid="orders-page">
      <div className="max-w-4xl mx-auto py-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-8" style={{ fontFamily: 'Dancing Script, cursive' }}>
          Mes Commandes
        </h1>
          ...
          <div className="space-y-4">
            {orders.map((order) => (
              <div key={order.id} className="card-service rounded-xl p-6" data-testid={`order-${order.id}`}>
                {/* Header */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 pb-4 border-b border-pink-100">
                  <div>
                ...
                <div className="space-y-3 mb-4">
                  {order.items.map((item, index) => (
                    <div key={index} className="flex justify-between items-center p-3 bg-pink-50/50 rounded-lg">
                      <div>
                ...
                {/* Footer */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-pink-100">
                  <Link 
                    to={`/entreprise/${order.enterprise_id}`}
                    className="flex items-center gap-2 text-pink-500 hover:text-[#2E74D6] transition-colors"
                  >
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-2xl font-bold text-sky-400">{order.total.toFixed(2)} CHF</p>
                  </div>
                </div>
