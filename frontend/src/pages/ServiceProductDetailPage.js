...
const ServiceProductDetailPage = () => {
  ...
  const getCategoryLabel = (categoryId) => {
    const categories = {
      video: 'videos',
      soinsd_esthetiques: 'Soinyys esthétiques',
  ...
  return (
    <div className="min-h-screen bg-[#FFF5F9] pt-24" data-testid="service-product-detail-page">
      {/* Breadcrumb */}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="space-y-4">
            <div className="aspect-square rounded-2xl overflow-hidden bg-pink-50/50">
              <img 
                ...
                {images.map((img, idx) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden border-2 transition-all ${
                      selectedImage === idx ? 'border-pink-400' : 'border-transparent opacity-60 hover:opacity-100'
                    }`}
                  >
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-3">
              <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                isService ? 'bg-pink-500/20 text-pink-500' : 'bg-sky-400/20 text-sky-400'
              }`}>
                {isService ? 'Service' : 'Produit'}
              </span>
              <span className="px-3 py-1 rounded-full text-xs font-medium bg-pink-50 text-gray-300">
            ...
            {/* Title */}
            <h1 className="text-3xl md:text-4xl font-bold text-white" style={{ fontFamily: 'Dancing Script, cursive' }}>
              {item.name}
            </h1>
            ...
            {enterprise && (
              <Link 
                ...
                className="flex items-center gap-3 p-4 bg-pink-50/50 rounded-xl hover:bg-pink-50 transition-colors"
              >
                <div className="w-12 h-12 bg-gradient-to-br from-pink-400 to-[#D4AF37] rounded-full flex items-center justify-center text-white font-bold">
                ...
                {enterprise.is_certified && (
                  <span className="px-2 py-1 bg-pink-500 text-white text-xs rounded-full">Certifié</span>
                )}
              </Link>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <div className="flex items-center border border-pink-100 rounded-xl overflow-hidden">
                <button 
                  ...
                  className="px-4 py-3 text-white hover:bg-pink-50 transition-colors"
                >
                  -
                </button>
                <span className="px-6 py-3 text-white font-medium bg-pink-50/50">{quantity}</span>
                <button 
            ...
            <div className="flex gap-3 pt-2">
              <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-colors ${
                  isInWishlist 
                    ...
                    : 'bg-pink-50/50 text-gray-400 hover:text-white hover:bg-pink-50'
                }`}
              >
              ...
              <button 
                onClick={handleShare}
                className="flex items-center gap-2 px-4 py-2 bg-pink-50/50 rounded-xl text-gray-400 hover:text-white hover:bg-pink-50 transition-colors"
              >
        ...
        {/* Enterprise Contact */}
        {enterprise && (
          <div className="mt-16 p-6 bg-pink-50/50 rounded-2xl">
            <h2 className="text-xl font-bold text-white mb-6" style={{ fontFamily: 'Dancing Script, cursive' }}>
        ...
        {reviews.length > 0 && (
          <div className="mt-16">
            <h2 className="text-2xl font-bold text-white mb-8" style={{ fontFamily: 'Dancing Script, cursive' }}>
              Avis clients ({reviews.length})
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {reviews.slice(0, 4).map((review) => (
                <div key={review.id} className="p-6 bg-pink-50/50 rounded-xl text-center">
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-center gap-1 mt-1">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <Star 
                            key={star} 
                            className={`w-4 h-4 ${star <= review.rating ? 'text-sky-400 fill-[#D4AF37]' : 'text-gray-600'}`} 
                          />
                        ))}
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {relatedItems.map((relatedItem) => (
                <Link 
                  ...
                  className="block bg-pink-50/50 rounded-xl overflow-hidden hover:bg-pink-50 transition-all group"
                >
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-sky-400 font-bold mt-1">{relatedItem.price?.toFixed(2)} CHF</p>
                  </div>
                </Link>
              ))}
            </div>
