...
const NotificationItem = ({ notification, onMarkRead, onDelete }) => {
  ...
  return (
    <div 
      className={`p-4 border-b border-white/5 hover:bg-pink-50/50 transition-colors ${!notification.is_read ? 'bg-pink-500/5' : ''}`}
          ...
          <div className="flex items-center gap-2 mt-2">
            {!notification.is_read && (
              <button
                ...
                className="text-xs text-pink-500 hover:text-[#0056D2] flex items-center gap-1"
              >
...
const NotificationCenter = ({ 
  ...
  return (
    <div className="fixed inset-0 z-50" onClick={onClose} data-testid="notification-center">
      ...
      <div 
        className="absolute right-4 top-20 w-full max-w-md bg-pink-50 rounded-2xl border border-pink-100 shadow-2xl overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-4 border-b border-pink-100 bg-gradient-to-r from-pink-400/10 to-transparent">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button 
                onClick={onClose}
                className="p-1 text-gray-400 hover:text-white rounded-lg hover:bg-pink-50"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
...
export const NotificationBadge = ({ count = 0, onClick }) => {
  ...
  return (
    ...
    >
      ...
      <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] flex items-center justify-center bg-pink-500 text-white text-xs rounded-full px-1 animate-pulse">
        {count > 99 ? '99+' : count}
      </span>
    </button>
  );
