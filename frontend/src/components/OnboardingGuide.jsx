...
const OnboardingGuide = ({ onComplete, userType = 'client' }) => {
  ...
  useEffect(() => {
    ...
    const onboardingDone = localStorage.getItem(`pinkgirl_onboarding_${userType}`);
    if (!onboardingDone) {
      setIsVisible(true);
    }
  }, [userType]);
  ...
  const clientSteps = [
    ...
    },
    {
      ...
      description: "Gagnez de l'argent sur chaque achat effectué sur PinkGirl !",
      icon: Gift,
  ...
  const enterpriseSteps = [
    {
      title: "Bienvenue sur PinkGirl Pro !",
  ...
  const handleComplete = () => {
    localStorage.setItem(`pinkgirl_onboarding_${userType}`, 'true');
    setIsVisible(false);
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg bg-[#0a0a0f] rounded-2xl border border-pink-100 shadow-2xl overflow-hidden">
        {/* Progress Bar */}
        <div className="h-1 bg-pink-50">
          <div 
        ...
        <div className={`h-40 bg-gradient-to-r ${currentStepData.color} relative overflow-hidden`}>
          ...
          <div className="absolute -bottom-10 -left-10 w-32 h-32 bg-pink-50 rounded-full" />
        ...
        <div className="p-6">
          ...
          {currentStepData.action && (
            <a
              ...
              className="block w-full text-center py-3 bg-pink-50 hover:bg-white/20 rounded-xl text-white font-medium transition-colors mb-4"
            >
