...
const sampleReviews = [
  ...
  },
  {
    ...
    rating: 5,
    text: 'La plateforme PinkGirl m\'a permis de découvrir de superbes prestataires locaux. Merci !',
    date: '5 Jan 2026',
...
const ReviewCard = ({ review }) => (
  <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-3 sm:p-5 min-w-[260px] sm:min-w-[320px] max-w-[260px] sm:max-w-[320px]" data-testid={`review-card-${review.id}`}>
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-0.5 flex-shrink-0">
            {[...Array(5)].map((_, i) => (
              <Star 
                key={i} 
                className={`w-3 h-3 sm:w-4 sm:h-4 ${i < review.rating ? 'text-sky-400 fill-[#D4AF37]' : 'text-gray-600'}`} 
              />
            ))}
          </div>
        </div>
    ...
    <div className="relative">
      <Quote className="absolute -top-1 sm:-top-2 -left-1 sm:-left-2 w-4 h-4 sm:w-6 sm:h-6 text-pink-500/20" />
    ...
    <section className="py-8 sm:py-16 md:py-24 bg-gray-50 overflow-hidden" data-testid="scrolling-reviews-section">
      <div className="max-w-7xl mx-auto px-4 md:px-8 mb-6 sm:mb-10">
        <div className="flex items-start sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-bold text-gray-900" style={{ fontFamily: 'Dancing Script, cursive' }}>
              {title}
            </h2>
