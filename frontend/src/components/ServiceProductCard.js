...
const enterpriseName = item.enterprise_name || "Entreprise supprimée";
const defaultImage = type === 'service' 
  ? 'https://images.unsplash.com/photo-1521737711867-e3b97375f902?w=800'
  : 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=800';
// Remplace uniquement cette fonction par celle-ci
const getImageUrl = () => {
  const image =
  ...
  // URL complète déjà correcte
  if (/^https?:\/\//i.test(cleanImage)) {
    return cleanImage;
  }

  // 🔥 IMPORTANT : ton backend utilise /api/uploads/
  if (cleanImage.startsWith('/api/')) {
...
<Link to={`/${type}/${id}`} className="block relative h-44 sm:h-52 overflow-hidden">
  ...
  />
      ...
      <div className="p-4">
        <Link to={`/${type}/${id}`}>
          <h3 className="text-base font-semibold text-gray-900 group-hover:text-pink-500 transition-colors mb-1 line-clamp-1">
            {name}
          </h3>
        </Link>
