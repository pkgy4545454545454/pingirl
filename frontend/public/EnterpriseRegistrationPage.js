...
const EnterpriseRegistrationPage = () => {
  ...
  const [step, setStep] = useState(1); // 1: Select enterprise, 2: Fill form, 3: Success, 4: New enterprise form
  ...
  const handleCloseBenefitsPopup = () => {
    ...
    if (selectedEnterprise) {
      ...
      setStep(4); // Go to new enterprise form
    }
  };

  // Handle new enterprise registration
  ...
  // Submit new enterprise registration
  const handleSubmitNewEnterprise = async (e) => {
    e.preventDefault();
    ...
    try {
      setSubmitting(true);
      await axios.post(`${API_URL}/auth/register-new-enterprise`, {
        // Enterprise info
      ...
      console.error('Error registering new enterprise:', error);
      toast.error(error.response?.data?.detail || 'Erreur lors de l\'inscription');
    } finally {
      setSubmitting(false);
    }
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-400 text-lg">
              Activez votre entreprise existante ou inscrivez votre nouvelle entreprise
            </p>
          </div>

          {/* Two Options */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
            {/* Option 1: Activate existing enterprise */}
            <div className="bg-gradient-to-br from-[#0047AB]/20 to-[#0047AB]/5 border border-[#0047AB]/30 rounded-2xl p-6 text-center">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Activer mon entreprise</h3>
              <p className="text-gray-400 text-sm mb-4">
                Votre entreprise est déjà listée sur Titelli ? Recherchez-la ci-dessous pour l'activer.
              </p>
              <span className="text-[#0047AB] text-sm font-medium">↓ Recherchez ci-dessous</span>
            </div>

            {/* Option 2: Register new enterprise */}
            <div 
              onClick={handleNewEnterprise}
              className="bg-gradient-to-br from-[#D4AF37]/20 to-[#D4AF37]/5 border border-[#D4AF37]/30 rounded-2xl p-6 text-center cursor-pointer hover:border-[#D4AF37]/50 transition-all group"
            >
              <div className="w-16 h-16 bg-[#D4AF37]/20 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform">
                <Sparkles className="w-8 h-8 text-[#D4AF37]" />
              </div>
              <h3 className="text-xl font-bold text-white mb-2">Inscrire mon entreprise</h3>
              <p className="text-gray-400 text-sm mb-4">
                Votre entreprise n'est pas encore listée ? Créez votre fiche entreprise maintenant.
              </p>
              <span className="inline-flex items-center gap-2 px-4 py-2 bg-[#D4AF37] text-black rounded-lg font-medium group-hover:bg-[#D4AF37]/90 transition-colors">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <span className="text-gray-500 text-sm">OU ACTIVEZ VOTRE ENTREPRISE</span>
            <div className="flex-1 h-px bg-white/10"></div>
          </div>
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div className="text-center py-12 text-gray-400">
                ...
                <p className="mb-4">Aucune entreprise trouvée</p>
                <button
                  onClick={handleNewEnterprise}
    ...
    return (
      <>
        ...
        <NewEnterpriseForm 
          ...
          onSubmit={handleSubmitNewEnterprise}
          onBack={() => {
            setStep(1);
            setIsNewEnterprise(false);
          }}
  ...
  return (
    <div className="min-h-screen bg-[#050505] pt-24 px-4 flex items-center justify-center">
      <div className="max-w-lg mx-auto text-center">
        ...
        {/* Status badge for new enterprise */}
        {isNewEnterprise && (
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 mb-4">
            <p className="text-yellow-400 text-sm font-medium">
              📋 Statut de votre entreprise : <span className="font-bold">EN ATTENTE</span>
            </p>
            <p className="text-gray-400 text-xs mt-1">
              Votre fiche entreprise sera visible après validation
            </p>
          </div>
        )}
...
// Step 4: New Enterprise Form (shown when isNewEnterprise is true and step is 4)
const NewEnterpriseForm = ({ formData, setFormData, managers, submitting, onSubmit, onBack, handleFileChange }) => {
  const categories = [
    'Restauration',
    'Coiffeurs', 
    'Soins esthétiques',
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">
            Créez votre fiche entreprise - Elle sera en attente de validation
          </p>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-gray-400 text-sm mb-2">Nom de l'entreprise *</label>
                <input
                  type="text"
                  required
                  value={formData.business_name}
                  onChange={(e) => setFormData({ ...formData, business_name: e.target.value })}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#D4AF37]/50"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#D4AF37]/50"
                  >
                    <option value="" className="bg-[#0F0F0F]">Sélectionnez</option>
                    {categories.map((cat) => (
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#D4AF37]/50"
                    placeholder="Ex: Pizzeria, Coiffeur homme..."
                  />
                </div>
              </div>
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#D4AF37]/50 h-24 resize-none"
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-gray-400 text-sm mb-2">Pays</label>
                  <input
                    type="text"
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, commerce_register_id: e.target.value })}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-[#D4AF37]/50"
                  placeholder="CHE-XXX.XXX.XXX (optionnel)"
                />
              </div>
              <div>
                <label className="block text-gray-400 text-sm mb-2">Pièce d'identité</label>
                <div className="relative">
                  <input
                    type="file"
                    accept=".pdf,.jpg,.jpeg,.png"
                  ...
                  />
                  <label
                    htmlFor="identity-doc-new"
                    className="flex items-center justify-center gap-2 w-full px-4 py-4 bg-white/5 border border-dashed border-white/20 rounded-lg text-gray-400 hover:border-[#D4AF37]/50 hover:text-white cursor-pointer transition-colors"
                  >
                    <Upload className="w-5 h-5" />
                    {formData.identity_document ? 'Document téléchargé ✓' : 'Cliquez pour télécharger'}
                  </label>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-4 bg-gradient-to-r from-[#D4AF37] to-[#B8960C] text-black font-semibold rounded-xl hover:opacity-90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {submitting ? (
              <>
